...
- **Project metadata**
  ...
  - `docs/`: A simple, static GitHub Pages-ready homepage (served from `/docs`) that explains the public catalog in non-technical terms and links back to DOJ sources. The site does **not** host documents; it only displays metadata and transparent DOJ links. The UI assets live under `docs/site/`, while `docs/index.html` points at those assets.
    - Homepage catalog cards now validate DOJ URLs and show a clear **“Source link pending”** label when a link is missing or malformed, so non-technical readers know why a link is unavailable.
    - `docs/name-search.html` + `docs/name-search.js`: A public-friendly name lookup page that shows **only** document metadata (dataset, page numbers, and counts) and always links back to justice.gov, so non-technical reviewers can verify mentions without viewing extracted text. The page now includes a **name dropdown** plus a **Search** button, so reviewers can pick a known person and clearly see when results update.
...
- **Streamlit dashboards (analysis + QA)**
  ...
  - `analysis/streamlit/pages/07_Name_Search_Mock.py`: Mock **Name Search** page for testing the public-safe name mentions experience. It ships with sample data so non-technical reviewers can validate the layout, search filters, and CSV export flow before the live name index is available. It now includes a **name dropdown** plus an explicit **Search** button so reviewers can choose a specific person and understand when filters are applied.
