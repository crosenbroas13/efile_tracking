...
- **Human-friendly run IDs**: the `<run_id>` now starts with the **main folder name you scanned** (sanitized for safe filenames), then the run type and timestamp. This puts the pull name first so non-technical reviewers can tell which inventory belongs to which drop at a glance.
- **Main folder date naming (recommended)**: name the top-level folder with the DOJ pull date (for example, `DOJ_DataSets_12.23.25`). That date becomes part of every run ID, so dashboards and logs clearly show which release is the latest.
- **Volume-based folder labeling**: if the folder tree includes a segment like `VOL00007`, the inventory treats that as **VOL00007** for every file beneath it. This keeps volume labels consistent even when files are nested deeper than one folder.
...
- **Matching probe run IDs**: probe run folders now start with the same **main folder name** captured from the inventory summary, then the run type and timestamp. This keeps inventory and probe outputs aligned for the same DOJ pull.
...
- **Home page (executive snapshot):** the landing view now auto-loads the *latest inventory and probe* and presents a **non-technical summary**:
  ...
  - **VOL Folder Structure**: a pie chart of file share by top-level folder plus a **Top 5 largest folders** table (so reviewers see where storage is concentrated).
...
- `outputs/run_index.json`: a **centralized “latest run” index** keyed by the original DOJ pull folder you scanned. Each entry records the most recent inventory **and** probe run for that pull so repeated runs update one place without overwriting history.
...
- **Windows/Mac absolute paths**: the CLI resolves `~` and relative paths; prefer absolute paths if you keep DOJ pull folders outside the repo.
...
- **Project metadata**
  ...
  - `docs/`: A simple, static GitHub Pages-ready homepage (served from `/docs`) that explains the public catalog in non-technical terms and links back to DOJ sources. The site does **not** host documents; it only displays metadata and transparent DOJ links. The UI assets live under `docs/site/`, while `docs/index.html` points at those assets.
    - Homepage now shows an **executive breakdown** (mirroring the Streamlit landing page) so non-technical readers can see **overall volume, VOL mix, file types, and source-link readiness** before diving into individual documents. It also includes **charts** and a **full document list** so every published item is visible and easy to audit without opening files.
    - `docs/name-search.html` + `docs/name-search.js`: A public-friendly name lookup page that shows **only** document metadata (VOL folder, page numbers, and counts) and always links back to justice.gov, so non-technical reviewers can verify mentions without viewing extracted text.
    - `docs/inventory-delta.html` + `docs/inventory-delta.js`: A **plain-language change tracker** that compares the **first** and **latest** inventory/probe pulls. It highlights top-level folder growth, storage shifts, and changes in text-ready PDFs so reviewers can explain *what changed* between runs without opening any files.
    ...
    - `docs/data/inventory_probe_delta.json`: The summary file powering the Inventory & Probe Delta page. It records first/last run IDs plus top-level folder changes in counts and sizes, helping stakeholders understand whether a VOL folder grew, shrank, or shifted.
...
- **Streamlit dashboards (analysis + QA)**
  ...
  - `analysis/streamlit/pages/07_Name_Search_Mock.py`: Mock **Name Search** page for testing the public-safe name mentions experience. It ships with sample data so non-technical reviewers can validate the layout, search filters, and CSV export flow before the live name index is available.
...
- **Core package (`src/` folder)**
  ...
  - `src/inventory.py`: Walks the DOJ pull folder, gathers file metadata and hashes, and also lists files inside ZIP archives without extracting them.
