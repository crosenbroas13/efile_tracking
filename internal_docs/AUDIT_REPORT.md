...
- **Probe helper script**: `pipelines/base/scripts/run_probe.py` is a thin wrapper that instantiates `ProbeConfig` and calls `run_probe_and_save` with two constants.【F:pipelines/base/scripts/run_probe.py†L1-L17】
- **Streamlit apps**: multipage launcher at `analysis/streamlit/Home.py` with pages `analysis/streamlit/pages/01_Inventory_QA.py` and `analysis/streamlit/pages/02_Probe_QA.py`; legacy single-page dashboard `analysis/streamlit/qa_fileimport.py` imports `src` modules directly to visualize inventories.【F:analysis/streamlit/qa_fileimport.py†L1-L69】
...
- Streamlit dashboards load artifacts directly from `outputs/` using helpers in `src/io_utils.py` and `src/probe_io.py`, assuming the current flat layout (inventory.csv at root; probe outputs under `outputs/probes/<run_id>`).【F:analysis/streamlit/qa_fileimport.py†L10-L41】【F:src/probe_io.py†L32-L62】
...
- Scripts live in both `src/` (`main.py`) and `pipelines/base/scripts/` (`run_probe.py`), leading to inconsistent locations for PyCharm users.【F:src/main.py†L1-L56】【F:pipelines/base/scripts/run_probe.py†L1-L17】
...
- Streamlit pages assume the flat `outputs/inventory.csv` location and do not handle versioned inventory/probe directories, so future reorganizations would break without compatibility layers.【F:analysis/streamlit/qa_fileimport.py†L10-L41】【F:analysis/streamlit/pages/02_Probe_QA.py†L1-L40】
...
- Keep Streamlit multipage app under `analysis/streamlit/` with pages that only read stored outputs; keep thin scripts in `pipelines/base/scripts/` that call into the package.
...
5. Add thin PyCharm scripts in `pipelines/base/scripts/run_inventory.py` and `pipelines/base/scripts/run_probe.py` that edit two constants and delegate to the package.
